...
module "webserver_security_group_rules_ingress" {
  ...
  rule_portocol  = "tcp"
  rule_cidr      = ["0.0.0.0/0"]
  rule_sec_grp   = module.webserver_security_group.webapp_sec_grp_Id
}

module "webserver_security_group_rules_egress" {
  source         = "./modules/security_group_rules"
  rule_type      = "egress"
  rule_from_port = "0"
  rule_to_port   = "65535"
...
module "webserver_lc" {
  ...
  security_group_Id = [ module.webserver_security_group.webapp_sec_grp_Id] 
  instance_Type     = "t2.micro"
  lc_name_prefix    =  "webserver" 
